...
import scala.slick.jdbc.Insert
...
trait JdbcStatementBuilderComponent { driver: JdbcDriver =>
  ...
  class InsertBuilder(val node: Node) {
    ...
    protected[this] val Insert(_, table: TableNode, ProductNode(columns)) = node
    ...
    def buildReturnColumns(node: Node, table: String): IndexedSeq[FieldSymbol] = {
      val ResultSetMapping(_, Insert(_, ktable: TableNode, ProductNode(kpaths)), _) =
