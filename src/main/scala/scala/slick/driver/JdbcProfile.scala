...
trait JdbcProfile extends SqlProfile with JdbcTableComponent
  ...
  class Implicits extends ImplicitJdbcTypes with ExtensionMethodConversions {
    ...
    implicit def columnBaseToInsertInvoker[T](c: ColumnBase[T]) = createCountingInsertInvoker(insertStatementCompiler.run(Node(c)).tree, ShapedValue.createShapedValue(c))
    implicit def shapedValueToInsertInvoker[T, U](u: ShapedValue[T, U]) = createCountingInsertInvoker(insertStatementCompiler.run(u.packedNode).tree, u)
